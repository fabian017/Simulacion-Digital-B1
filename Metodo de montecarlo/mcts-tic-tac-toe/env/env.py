...
class GameState:
    def __init__(self):
        ...
        pygame.display.set_caption('Juego de montecarlos')
    ...
    # Display title
    def title_msg(self):
        titleSurf = TITLE_FONT.render('Juego de montecarlo', True, WHITE)
    ...
    # Display rule
    def rule_msg(self):
        ruleSurf1 = BASIC_FONT.render('Ganador: O or X',
                                      True, WHITE)
    ...
    def score_msg(self):
        ...
        scoreSurf4 = BASIC_FONT.render('Empate = ' + str(self.count_draw), True,
                                       WHITE)
    ...
    def turn_msg(self):
        if self.turn == 0:
            turnSurf = BASIC_FONT.render("Juega O!", True, WHITE)
            turnRect = turnSurf.get_rect()
            turnRect.topleft = (MARGIN, 135)
            DISPLAYSURF.blit(turnSurf, turnRect)
        else:
            turnSurf = BASIC_FONT.render("Juego X!", True, WHITE)
            turnRect = turnSurf.get_rect()
    ...
    def display_win(self, win_index):
        ...
        if win_index == 1:
            ...
            winSurf = GAMEOVER_FONT.render("Gana O!", True, BLACK)
        ...
        if win_index == 2:
            ...
            winSurf = GAMEOVER_FONT.render("Gana X!", True, WHITE)
            winRect = winSurf.get_rect()
            winRect.midtop = (WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2 - 50)
            DISPLAYSURF.blit(winSurf, winRect)
            pygame.display.update()
        ...
        if win_index == 3:
            ...
            winSurf = GAMEOVER_FONT.render("Empate!", True, BLACK)
            winRect = winSurf.get_rect()
            winRect.midtop = (WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2 - 50)
            DISPLAYSURF.blit(winSurf, winRect)
            pygame.display.update()
